...
extern "C" const unsigned char model_int8_vela_tflite[];
...
TfLiteStatus ProfileMemoryAndLatency() {
  ...
  tflite::RecordingMicroInterpreter interpreter(
      tflite::GetModel(&model_float_tflite[0]), op_resolver, allocator,
...
TfLiteStatus LoadFloatModelAndPerformInference() {
  const tflite::Model* model =
      ::tflite::GetModel(model_float_tflite);
  ...
  for (int i = 0; i < kNumTestValues; ++i) {
    ...
    printf("Input [%.3f] = %.3f / Delta %.3f\n\r" , sin(golden_inputs[i]),y_pred,abs(sin(golden_inputs[i]) - y_pred));
  }
...
TfLiteStatus LoadQuantModelAndPerformInference() {
  ...
  for (int i = 0; i < kNumTestValues; ++i) {
    ...
    printf("Input [%.3f] = %.3f / Delta %.3f\n\r" , sin(golden_inputs_float[i]), y_pred ,abs(sin(golden_inputs_float[i]) - y_pred));
  }

  return kTfLiteOk;
}

TfLiteStatus LoadVelaModelAndPerformInference() {
  // Map the model into a usable data structure. This doesn't involve any
  // copying or parsing, it's a very lightweight operation.
  const tflite::Model* model =
      ::tflite::GetModel(model_int8_tflite);
...
int app_main(void) { 
  ...
  printf("(INFO) Load Float Model and Perform Inference\r\n");
  TF_LITE_ENSURE_STATUS(LoadFloatModelAndPerformInference());

  printf("(INFO) Load Quantized Model and Perform Inference\r\n");
  TF_LITE_ENSURE_STATUS(LoadQuantModelAndPerformInference());
  
  printf("(INFO) Load Vela Model and Perform Inference\r\n");
  TF_LITE_ENSURE_STATUS(LoadVelaModelAndPerformInference());
